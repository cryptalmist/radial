...
public class RadialSlot {
    ...
    public ItemStack getRenderStack() {
        ...
        try {
            ...
            ItemStringReader.ItemResult result = new ItemStringReader(registryLookup).consume(new StringReader(itemId));

            cachedStack = new ItemStack(result.item(), 1);
            cachedStack.applyUnvalidatedChanges(result.components());
        } catch (Exception e) {
            cachedStack = new ItemStack(Items.BARRIER);
            cachedStack.set(DataComponentTypes.CUSTOM_NAME, Text.literal("Invalid Item ID"));
        }
