...
public abstract class GridIconTab<T> implements IconTab {
    ...
    protected abstract void renderIcon(
            DrawContext graphics, int x, int y, int mouseX, int mouseY, T item, boolean hovered);

    protected abstract void selectIcon(T item);

    protected abstract Text getItemNarration(T item);
    ...
    public void setup(int width, int height, Consumer<Drawable> addRenderable, Consumer<Element> addWidget) {
        ...
        listWidget = new IconGridList(MinecraftClient.getInstance(), listWidth, Math.max(1, bottom - top), top, 24);
    ...
    private class IconGridList extends AlwaysSelectedEntryListWidget<IconGridEntry> {

        public IconGridList(MinecraftClient mc, int w, int h, int y, int rowHeight) {
    ...
    private class IconGridEntry extends AlwaysSelectedEntryListWidget.Entry<IconGridEntry> {
        ...
        public boolean mouseClicked(Click click, boolean doubleClick) {
            ...
            for (int i = 0; i < items.size(); i++) {
                int x = getX() + i * slotSize;
                int y = getY() + verticalOffset;

                if (click.x() >= x && click.x() < x + slotSize && click.y() >= y && click.y() < y + slotSize) {
        ...
        public Text getNarration() {
            return items.isEmpty() ? Text.literal("Empty row") : getItemNarration(items.getFirst());
        }
    }
}
